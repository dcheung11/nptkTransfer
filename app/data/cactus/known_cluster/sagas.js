...
export function* fetchAllKnownClusters(data) {
  try {
    const response = yield call(() =>
      request(
        `https://cactus.magarveylab.ca/api/known_cluster/query?select=*,smallmolecule(*)&limit=null`,
        {
          method: 'get',
          headers: {
            Accept: 'application/json',
            'Content-Type': 'application/json',
            Authorization: `Auth0 ${data.bearerData.bearerToken}`,
          },
        },
      ),
    );
    yield put(successAllKnown(response));
  } catch (err) {
    yield put(failedAllKnown(err));
  }
}
...
export function* fetchClusterInterpro(data) {
  try {
    const response = yield call(() =>
      request(
        `https://cactus.magarveylab.ca/api/known_cluster/id/${
...
export default function* clusterSaga() {
  yield all([
    ...
    takeLatest(FETCH_ALL_KNOWN, fetchAllKnownClusters),
